...
func Prepare(_engine int, _address string, _scope string, _uname string, _accessKey string, _accessSecret string) (string, error) {
	...
	case proto.Engine_ENGINE_MINIO:
		return prepareMinio(_address, _scope, _uname, _accessKey, _accessSecret)
	}
...
// 生成一个永久有效链接
func Publish(_engine int, _address string, _scope string, _uname string, _accessKey string, _accessSecret string) (string, error) {
	switch proto.Engine(_engine) {
	case proto.Engine_ENGINE_QINIU:
		return publishQiniu(_address, _scope, _uname, _accessKey, _accessSecret)
	case proto.Engine_ENGINE_MINIO:
		return publishMinio(_address, _scope, _uname, _accessKey, _accessSecret)
	}
	return "", errors.New("unsupported engine")
}

// 生成一个有效期链接
func Preview(_engine int, _address string, _scope string, _uname string, _expiry uint64, _accessKey string, _accessSecret string) (string, error) {
	switch proto.Engine(_engine) {
	case proto.Engine_ENGINE_QINIU:
		return previewQiniu(_address, _scope, _uname, _expiry, _accessKey, _accessSecret)
	case proto.Engine_ENGINE_MINIO:
		return previewMinio(_address, _scope, _uname, _expiry, _accessKey, _accessSecret)
	}
	return "", errors.New("unsupported engine")
}
